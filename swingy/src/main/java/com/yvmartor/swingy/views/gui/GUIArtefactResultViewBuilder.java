...
public class GUIArtefactResultViewBuilder extends AdventureViewBuilder {

    private Font font = new Font("Courier", Font.ITALIC, 20);
    private String[] continue_button = {"Continue"};
    private boolean keep;

    public GUIArtefactResultViewBuilder(boolean keep){
        this.keep = keep;
    }
    ...
    public void buildTitle() {
        if (keep == true)
            adventureView.getTitle().setText("You keep the Artefact");
        else
            adventureView.getTitle().setText("You throw the Artefact");
