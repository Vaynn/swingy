...
public class GUIDidIRunViewBuilder extends AdventureViewBuilder
{
    ...
    public GUIDidIRunViewBuilder(boolean escape){
        this.escape = escape;
    }
    ...
    public void buildTitle() {
        if (!escape) {
            adventureView.getTitle().setText("IMPOSSIBLE TO RUN");
            adventureView.getTitle().setForeground(Color.RED);
        } else if (escape){
            adventureView.getTitle().setText("TIME TO RUN");
    ...
    public void buildLambdaCard() {
        if (escape)
            adventureView.setLambda_card(new LambdaCard().
                    getLambdaCard("/images/run.jpg", "HURRY UP", ""));
        else
            adventureView.setLambda_card(new LambdaCard().
                    getLambdaCard("/images/cant_run.jpg", "YOU ARE TrAPpED", ""));
